! VARYING HOLE DISTANCE (26/02/2018) BENDING - [varying_hole_distance_plate_1.5_depth_1] -> 80x124mm

!The same CODE for Uniaxial and Biaxial cases, just comment or uncomment some commands
!lines: 17/18 , 55/56 , 315/321 , 327/333
...
!*enddo
				...
				*dim,wd,string,200,1
				wd(1,1)='C:\Manasses\varying_hole_distance_bending_80x124mm\bending_uniaxial'
				!wd(1,1)='C:\Manasses\varying_hole_distance_bending_80x124mm\bending_biaxial'
				...
				diff = 50 !mínimo 5.0 mm -> PROBLEM BORDER
				inicial = 56
				final = 72
				incr = 2
...
*do,diff,inicial,final,incr
	...
	*endif
	...
	parres,new,parameters,txt,'C:\Manasses\varying_hole_distance_bending_80x124mm\bending_uniaxial'
	!parres,new,parameters,txt,'C:\Manasses\varying_hole_distance_bending_80x124mm\bending_biaxial'
	...
	!INÍCIO DO BLOCO DE COMANDO DENTRO DO LOOP
		...
		*get,volume1,volu,0,num,max !it gets the volume number		
		block,x1-1,x2+1,y1-1,y2/2,z1+1,-Z2-1     
		allsel !select all volumes
		vsel,u,volu,,volume1 !unselect volume1
		*get,volume2,volu,0,num,max		!it gets the volume number	
		block,x2/2,x2+1,(y2/2)-1,y2+1,z1+1,-Z2-1 
		allsel !select all volumes
		vsel,u,volu,,volume1 !unselect volume1
		vsel,u,volu,,volume2 !unselect volume2
		*get,volume3,volu,0,num,max		!volume3 is select, it gets the volume number	
		...
		*get,teste,volu,0,num,max	!some volume is select, it gets the volume number, and stores in the variable "teste"	
		allsel !select all volumes
		
		vsbv,teste,volume3 !subtract volume3 from teste
		vplot
