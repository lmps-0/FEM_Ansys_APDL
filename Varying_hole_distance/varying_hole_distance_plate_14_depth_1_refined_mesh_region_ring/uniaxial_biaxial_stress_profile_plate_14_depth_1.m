...
clear all
close all
rmdir plots s % the 's' remove the folder and the sub folder or sub files
mkdir plots
...
for cont=1:1:t
    ...
    %figure('pos',[xpos(cont) ypos(cont) 650 500],'Name',upper(axial{cont}),'units','normalized','outerposition',[0 0 1 1]);
    ...
    for a=1:1:n %going through each point difference
        path=['C:\Manasses\varying_hole_distance_plate_14_depth_1_refined_mesh_region_ring\',axial{cont},'\difference',num2str(diff(a)),'mm\'];%address
