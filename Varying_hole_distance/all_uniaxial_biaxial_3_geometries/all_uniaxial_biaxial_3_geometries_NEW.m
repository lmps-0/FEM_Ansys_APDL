...
% The intention is to plot 2 charts, one UNIAXIAL, other BIAXIAL
% In each one the 3 geometries must be ploted.
format long
clear all
close all
...
%             hold on
            ...
            grid on
            point_A(temp+ttt)=plot(diff(temp),MATRIZSTRESST(1,2,temp),plotstyle_A{cont2},'markersize',markersize(cont2),'Linewidth',2.5)%% Percentage stress increase in comparison with the plate with one hole
            
            hold on
            point_B(temp+ttt)=plot(diff(temp),MATRIZSTRESST(2,2,temp),plotstyle_B{cont2},'markersize',markersize(cont2),'Linewidth',2.5)%% Percentage stress increase in comparison with the plate with one hole
