...
*enddo
	...
	*do,cont,1,2,1
		...
		!parsav,all,parameters_vget1,txt,wd(1,1)
		! /mkdir,%axial(1,cont)% !Creates the new folder for the current iteration
		! finish
		! /clear,,nopr
		! /prep7 
								
		!parres,new,parameters_vget1,txt,'C:\Manasses\center_hole'
		...
		*do,cont2,1,3,1
			...
			parsav,all,parameters_vget2,txt,wd(1,1)
			! /mkdir,plate_%plate(1,cont2)%_depth_%depth(1,cont2)% !Creates the new folder for the current iteration
			! finish
			! /clear,,nopr
			! /prep7 
					
			RESUME,%axial(1,cont)%_plate_%plate(1,cont2)%_depth_%depth(1,cont2)%,'db',%wd(1,1)%\%axial(1,cont)%\plate_%plate(1,cont2)%_depth_%depth(1,cont2)%
						
			!*dim,axial,string,200,3
			!axial(1,1) ='uniaxial'
			!axial(1,2) ='biaxial'
			!axial(1,3) ='uniaxial_r_eq_1'
			parres,new,parameters_vget2,txt,'C:\Manasses\center_hole'
